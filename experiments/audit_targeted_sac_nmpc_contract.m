...
assert(cfg.training.checkpointFrequencyEpisodes == 10, ...
    'Checkpoints must be saved every 10 episodes.');
...
assert(strcmp(cfg.observation.featureVersion, 'causal_feature_v2') && ...
    ...
    ~cfg.observation.includeSolveTime && ...
    ~cfg.observation.includeTeacherPrediction, ...
    'The approved causal SAC observation contract has changed.');
assert(strcmp(cfg.environment.positionTerminationPolicy, ...
    'nmpc_state_bounds_only') && isinf(cfg.environment.maxAbsolutePosition), ...
    'The obsolete absolute-position termination guard is active.');
...
artifact = load(bankPath, 'trainContextBank', 'trainContextManifest', ...
    ...
    'buildMetadata');
assert(~isempty(artifact.trainContextBank) && ...
    ~isempty(artifact.auxiliaryGrowthContextBank), ...
    'The active causal context-bank strata must be nonempty.');
...
assert(all(arrayfun(@has_causal_history, ...
    [artifact.trainContextBank; artifact.auxiliaryGrowthContextBank])), ...
    'A training context lacks causal feature history.');
...
assert(all(stageCounts > 0) && stageCounts(4) == ...
    numel(artifact.trainContextBank), ...
    'The causal context bank leaves a curriculum stage empty.');

verify_previous_input_smoothness(cfg.nmpc);

outputRoot = fullfile(projectRoot, cfg.targeted.resultRoot, ...
    'specialist_sac_nmpc_contract_audit_causal_v2');
...
write_text(fullfile(outputRoot, 'COMPLETED.txt'), sprintf( ...
    ...
    'feature_version=causal_feature_v2\n' ...
    'exitflag_zero_policy=strict_pending_closed_loop_validation\n'], ...
...
fprintf(fileId, '- Observation: 328D `causal_feature_v2`.\n');
fprintf(fileId, ['- Position termination: declared NMPC state bounds; ' ...
    'no independent absolute +/-20 m guard.\n']);
...
function yes = has_causal_history(item)
yes = isfield(item, 'FeatureVersion') && ...
