...
outputRoot = fullfile(projectRoot, cfg.resultRoot, ...
    'reference_feasibility_v2_realized_coverage');
...
for familyIndex = 1:numel(cfg.reference.families)
    ...
    for speed = cfg.reference.approvedIdSpeedAnchors
        for loadIndex = 1:numel(cfg.reference.approvedLoadNames)
            caseIndex = caseIndex + 1;
            accelerationTarget = quad_targeted_load_acceleration( ...
                speed, loadIndex, cfg.reference, family);
...
accelerationError = abs(metric.peakAcceleration - accelerationTarget) / ...
    accelerationTarget;
physical = metric.finite && speedError <= refCfg.speedRelativeTolerance && ...
    accelerationError <= refCfg.accelerationRelativeTolerance && ...
...
row.AccelerationRelativeError = accelerationError;
...
fprintf(fileId, '- Load rule: `%s`.\n', ...
    cfg.reference.loadAccelerationRule);
fprintf(fileId, ['- Realized speed/acceleration relative tolerances: ' ...
    '`%.1f%% / %.1f%%`.\n'], ...
    100 * cfg.reference.speedRelativeTolerance, ...
    100 * cfg.reference.accelerationRelativeTolerance);
