...
artifact = load(bankPath, 'trainContextBank', ...
    ...
    'auxiliaryGrowthContextManifest', 'buildMetadata');
hardBank = artifact.trainContextBank;
growthBank = artifact.auxiliaryGrowthContextBank;
bank = [hardBank; growthBank];
for index = 1:numel(bank)
    bank(index).ContextIndex = index;
end
cellIds = unique(string(artifact.trainContextManifest.CellId), 'stable');
assert(~isempty(hardBank) && numel(hardBank) == ...
    height(artifact.trainContextManifest), ...
    'The hard-event context bank is empty or inconsistent.');
assert(numel(growthBank) == ...
    height(artifact.auxiliaryGrowthContextManifest), ...
    'The growth-only auxiliary bank is inconsistent.');
assert(all([hardBank.PreDivergenceEligible]), ...
    'SAC bank contains a context without a valid pre-divergence LQR event.');
assert(all([hardBank.HardEvent]) && ~any([hardBank.GrowthOnly]), ...
    'The hard SAC stratum contains a growth-only context.');
assert(all([growthBank.GrowthOnly]) && ~any([growthBank.HardEvent]) && ...
    ~any([growthBank.PreDivergenceEligible]), ...
    'The curriculum auxiliary stratum contains a hard-event context.');
counts = groupcounts(string(artifact.trainContextManifest.CellId));
assert(all(counts <= cfg.probe.maximumContextsPerPositiveFactorialCell), ...
    'A positive factorial cell exceeds the approved context cap.');
sourceCount = numel(bank);
if isfield(artifact, 'buildMetadata') && ...
        isfield(artifact.buildMetadata, 'sourceEpisodeCount')
    sourceCount = artifact.buildMetadata.sourceEpisodeCount;
end
summary = struct('SourceCount', sourceCount, ...
    'ActiveCount', numel(bank), 'HardCount', numel(hardBank), ...
