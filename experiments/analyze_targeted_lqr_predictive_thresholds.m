...
assert(~cfg.predictiveAnalysis.finalThresholdsLocked && ...
    ...
    'This diagnostic must run before final thresholds are locked.');
...
write_text(fullfile(temporaryRoot, 'COMPLETED.txt'), sprintf( ...
    ...
    'status=completed_diagnostic_thresholds_not_locked\n' ...
    'episodes=%d\ncandidate_steps=%d\nthreshold_candidates=%d\n' ...
    'hard_episodes=%d\nwall_seconds=%.6f\n'], ...
    numel(bank), metadata.candidateStepCount, height(candidates), ...
...
for offset = 1:horizon
    ...
    windows.safetyAny = windows.safetyAny | ...
...
rowCount = numel(grid.positionM) * numel(grid.attitudeDeg) * ...
    numel(grid.velocityMps) * numel(grid.growthFactor) * ...
    ...
    for attitude = grid.attitudeDeg
        for velocity = grid.velocityMps
            threshold = struct('positionM', position, ...
                ...
                'bodyRateRadps', cfg.threshold.bodyRateRadps);
            ...
            for growthFactor = grid.growthFactor
                for consecutive = grid.minimumConsecutiveSteps
                    cursor = cursor + 1;
                    rows(cursor) = summarize_candidate(base, growthFactor, ...
                        consecutive, cellGroup, familyGroup, cursor, threshold);
                end
            end
        end
    end
...
base.absoluteAny = windows.position >= threshold.positionM | ...
    windows.attitudeDeg >= threshold.attitudeDeg | ...
    windows.velocity >= threshold.velocityMps | ...
    windows.bodyRate >= threshold.bodyRateRadps;
...
for offset = 1:cfg.horizonSteps
    ...
    stateIndex = starts + offset;
    inputIndex = starts + offset - 1;
    eventNow = metric.position(:, stateIndex) >= threshold.positionM | ...
        metric.attitudeDeg(:, stateIndex) >= threshold.attitudeDeg | ...
        metric.velocity(:, stateIndex) >= threshold.velocityMps | ...
        metric.bodyRate(:, stateIndex) >= threshold.bodyRateRadps | ...
        metric.constraint(:, stateIndex) | ...
        ~metric.finite(:, stateIndex) | ...
        metric.saturated(:, inputIndex);
...
row.GrowthOnlyPositiveCount = nnz(positive & sustained & ...
...
fprintf(fileId, ['\nThe shortlist is diagnostic only. It selects the grid row ' ...
    ...
    'No NMPC, SAC, surrogate, test or OOD result entered this analysis.\n']);
clear cleanup;
end
...
row = struct('CandidateId', "", 'PositionThresholdM', NaN, ...
    ...
    'SafetyForecastPositiveCount', 0, 'GrowthOnlyPositiveCount', 0, ...
