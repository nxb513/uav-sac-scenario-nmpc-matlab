function episode = fixed_nmpc_rollout(x0, reference, thetaPlant, thetaModel, cfg, stepCount, plantDisturbanceSpec)
%FIXED_NMPC_ROLLOUT Receding-horizon fixed-model NMPC episode.
%
% thetaPlant is hidden from the controller. thetaModel is the fixed model
% used by NMPC, normally the nominal plant.

if nargin < 5 || isempty(cfg)
    cfg = step2_nmpc_config();
end
if nargin < 4 || isempty(thetaModel)
    thetaModel = cfg.plant.nominal;
end
...
if stepCount <= 0 || stepCount ~= floor(stepCount)
    error('fixed_nmpc_rollout:BadStepCount', ...
          'stepCount must be a positive integer.');
end

x0 = x0(:);
if numel(x0) ~= 12
    error('fixed_nmpc_rollout:BadStateSize', 'x0 must have 12 elements.');
end
...
if isnumeric(reference) && ~isempty(reference) && size(reference, 1) == 12 && size(reference, 2) > 1
    ...
    error('fixed_nmpc_rollout:MissingStepCount', ...
