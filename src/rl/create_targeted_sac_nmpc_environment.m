...
env = rlFunctionEnv(observationInfo, actionInfo, ...
    ...
    function [initialObservation, info] = reset_environment()
        ...
        info.FeatureVersion = scenario.FeatureVersion;
        ...
        info.StateHistory = scenario.StateHistory;
        info.InputHistory = scenario.InputHistory;
        info.PredictionResidual = scenario.PredictionResidual;
        ...
        initialObservation = targeted_sac_make_causal_observation( ...
            info.StateHistory, info.InputHistory, reference(:, 1:21), ...
            info.PredictionResidual, cfg);
        clear cleanup;
    end
    ...
    function [observation, reward, isDone, info] = ...
        ...
        try
            ...
            input = solution.u0;
            nominalPrediction = quad_step_rk4(time, info.State, input, ...
                cfg.environment.sampleTime, cfg.nmpc.plant.nominal, []);
            ...
            isDone = stepIndex >= cfg.environment.stepsPerEpisode || ...
                ...
                invalidEuler || constraintViolation;
        ...
        nextStateHistory = [info.StateHistory(:, 2:end), nextState];
        nextInputHistory = [info.InputHistory(:, 2:end), input];
        nextReferenceLookahead = info.Reference(:, ...
            stepIndex + 1:stepIndex + 21);
        observation = targeted_sac_make_causal_observation( ...
            nextStateHistory, nextInputHistory, nextReferenceLookahead, ...
            predictionResidual, cfg);
        ...
        info.Log.PredictionResidual(:, stepIndex) = predictionResidual;
        info.Log.ExceptionIdentifier{stepIndex} = exceptionIdentifier;
        info.StepIndex = stepIndex;
        info.State = nextState;
        info.StateHistory = nextStateHistory;
        info.InputHistory = nextInputHistory;
        info.PredictionResidual = predictionResidual;
...
function validate_causal_context(scenario, cfg)
required = {'StateHistory', 'InputHistory', 'PredictionResidual', ...
    'FeatureVersion'};
assert(all(isfield(scenario, required)), ...
    'The selected context does not contain causal feature history.');
assert(strcmp(scenario.FeatureVersion, cfg.observation.featureVersion), ...
    'Context and SAC feature versions differ.');
assert(isequal(size(scenario.StateHistory), [12, 4]) && ...
    isequal(size(scenario.InputHistory), [4, 4]) && ...
    isequal(size(scenario.PredictionResidual), [12, 1]), ...
    'The selected context has invalid causal feature dimensions.');
assert(all(isfinite(scenario.StateHistory), 'all') && ...
    all(isfinite(scenario.InputHistory), 'all') && ...
    all(isfinite(scenario.PredictionResidual)), ...
    'The selected context has nonfinite causal feature data.');
end
...
log.PredictionResidual = nan(12, stepCount);
...
if newFile
    fprintf(fileId, ['worker,episode,episode_seed,context_index,' ...
        'context_episode,context_class,feature_version,start_step,stage,reference,disturbance,level,' ...
        'scenario_seed,lqr_divergence,already_outside,' ...
        'predivergence_eligible,event_step,lead_steps,lqr_risk,' ...
        'lqr_log_energy_growth\n']);
end
fprintf(fileId, ...
    ['%d,%d,%d,%d,%d,%s,%s,%d,%d,%s,%s,%d,%d,%d,%d,%d,' ...
    '%.0f,%.0f,%.16g,%.16g\n'], ...
...
episodeLog.featureVersion = info.FeatureVersion;
