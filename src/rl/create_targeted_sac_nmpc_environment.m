...
if cfg.environment.forceCurriculumStage > 0
    ...
    localEpisodeBudget = ceil(cfg.training.transitionBudget / ...
        (cfg.environment.stepsPerEpisode * workerCount));
    transitionEpisodes = ceil( ...
        cfg.environment.curriculumTransitionFractions * localEpisodeBudget);
    stage = find(localEpisode <= [transitionEpisodes, inf], 1, 'first');
end
end
...
stratum = string({bank.UncertaintyStratum});
disturbanceType = disturbanceType(:);
disturbanceLevel = disturbanceLevel(:);
stratum = stratum(:);
switch stage
    case 1
        mask = disturbanceType == "zero" & stratum == "low";
    case 2
...
if newFile
    fprintf(fileId, ['worker,episode,episode_seed,context_index,' ...
        'context_episode,start_step,stage,reference,disturbance,level,' ...
        'scenario_seed\n']);
end
fprintf(fileId, '%d,%d,%d,%d,%d,%d,%d,%s,%s,%d,%d\n', ...
    info.WorkerId, info.EpisodeIndex, info.EpisodeSeed, ...
    info.ContextIndex, info.ContextEpisodeIndex, info.AbsoluteStartStep, ...
    info.StageIndex, info.ReferenceType, info.DisturbanceType, ...
    info.DisturbanceLevel, scenarioSeed);
clear cleanup;
end
